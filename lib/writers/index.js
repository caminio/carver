module.exports = function Writers( Carver ){
  ...
  Carver.prototype.contentArrayPromiseChain = function contentArrayPromiseChain(){
    ...
    var compiler = this;
    // compiler._skipDependencies = false;
    ...
    if( renderLangs.length < 1 || compiler.options.langlessCopy ){
      if( compiler.options.renderLangs.length > 0 )
        renderLangs = compiler.options.renderLangs;
      else
        renderLangs.push( undefined );
    }


    var allWrittenPromise = new RSVP.Promise( function(resolve){ resolve(); });
    renderLangs.forEach( function(lang){ 
      allWrittenPromise = allWrittenPromise.then( function(){ return compiler.contentLangWriter( lang ); });
    });

    allWrittenPromise = allWrittenPromise.then( function(){
      return compiler.runDependencies(); 
    });
    
    allWrittenPromise = allWrittenPromise.then(function(){
      return new RSVP.Promise(function(resolve){ resolve(compiler); });
    });
  ...
  Carver.prototype.contentLangWriter = function contentLangWriter( lang ){
    ...
    function attachPostprocessors(content){
      return compiler.createHookChain('after.write', content);
    }

    function writeToPaths(content){
      var writePromise = new RSVP.Promise( function(resolve){ resolve(); });
      compiler.options.destinations.forEach( function(dest){
    ...
    function checkAndWrite( destination, content, options ){
      ...
      return new RSVP.Promise( function( resolve ){
        ...
        if( compiler.options.doc && !options.draft && compiler.options.publishingStatusKey && compiler.options.doc[compiler.options.publishingStatusKey] !== 'published' )
          return resolve();
