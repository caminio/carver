...
module.exports = function CarverExport(){
  ...
  Carver.prototype.compileContent = function compileContent( content, options, cb ){
    ...
    function continueContent(err, result){
      ...
      if( compiler.config.locals.doc && compiler.config.locals.doc.filenamePrefix && !options.filenamePrefix )
  ...
  Carver.prototype.setupWorkdir = function setupWorkdir(){
    ...
    if( this.workdirSettings.draftsPath )
      this.config.draftsPath = this.workdirSettings.draftsPath;
  ...
  Carver.prototype.writeToDestination = function writeToDestination( htmlContent, options, cb ){
    ...
    // if publishKey is defined and doc replys false, skip public creation of document
    if( compiler.config.publishKey && 
        options.locale.doc && 
        (compiler.config.publishKey in options.locale.doc[compiler.config.publishKey]) &&
        !options.locale.doc[compiler.config.publishKey] )
      return checkAndCreateDrafts( null );

    Carver.writers[protocol]( htmlContent, (options.filename || null), _.merge( {  cwd: this.cwd, 
                                                                                   locale: options.locale,
                                                                                   logger: this.logger,
                                                                                   template: options.template }, this.workdirSettings ), checkAndCreateDrafts );

    function checkAndCreateDrafts( err ){
      if( err ){ return cb ? cb( err ) : err; }
      if( !( compiler.config.draftsPath && options.locals.doc && options.locals.doc[compiler.config.primaryKey] ) )
        return runDependenciesAndQuit( null );

      var filename = join( compiler.config.draftsPath, options.locals.doc[compiler.config.primaryKey].toString() );
      Carver.writers[protocol]( htmlContent, filename, _.merge( {  cwd: compiler.cwd, 
                                                               locale: options.locale,
                                                               logger: compiler.logger,
                                                               template: options.template }, compiler.workdirSettings, { destination: '' } ), runDependenciesAndQuit );
    ...
    function runDependenciesAndQuit( err ){
      ...
      if( compiler.config.controller && compiler.config.controller.dependencies && compiler.config.controller.dependencies instanceof Array ){
        ...
        return async.eachSeries( compiler.config.controller.dependencies, function( dep, nextIteration ){
          ...
          if( typeof(dep.compile) === 'function' )
            return dep.compile( compiler.constructor, _.merge({},options,{ skipDependencies: compiler.config.skipDependencies}), nextIteration );
    ...
    function runAfterWriter( err ){
      ...
      if( compiler.config.controller && typeof(compiler.config.controller.after) === 'function' )
        return compiler.config.controller.after( options, (cb ? cb : function(){}) );
      if(cb) cb( null );
    }
