...
module.exports = function SitterExport(){
  ...
  Sitter.init = function initSitter( options ){
    ...
    compiler.options.globals = compiler.options.globals || {};
    ...
    compiler.logger.debug('rocksol compiler initialized at ', compiler.cwd || ' string return level');
  ...
  Sitter.prototype.compileContent = function compileContent( content, options, cb ){
    ...
    function continueContent(err, result){
      ...
      if( self.workdirSettings && self.workdirSettings.destination )
        return typeof(cb) === 'function' ? cb( err, self.writeToDestination( result, options ) ) : self.writeToDestination( result, options );
  ...
  Sitter.prototype.setupWorkdir = function setupWorkdir(){
    ...
    if( !fs.existsSync( this.cwd ) )
      throw new Error('rocksol initialized with workdir but ' + this.cwd + ' was not found');
   ...
   */
  Sitter.prototype.writeToDestination = function writeToDestination( htmlContent, options ){
    
    var dst = this.workdirSettings.destination;
    var protocol = dst.indexOf('//') > 0 ? dst.split('//')[0] : 'native';

    if( !(protocol in Sitter.writers) )
      throw new Error('rocksol compiler could not find a matching writer for '+protocol );

    Sitter.writers[protocol]( htmlContent, null, _.merge( { globals: this.options.globals, 
                                                            cwd: this.cwd, 
                                                            locale: options.locale,
                                                            logger: this.logger,
                                                            template: options.template }, this.workdirSettings ) );
