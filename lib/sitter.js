...
module.exports = function SitterExport(){
  ...
  Sitter.prototype.compile = function compile( content, options, cb ){
    ...
    this.logger.debug('sitter compiling ', ( ( content && ( content.filename || content.template || content.length+' Bytes' ) ) ||  
                                               options.template || 'workdir') + ' with options' + options );

    options = options || {};
    options.engine = options.engine || 'plaintext';

    options.locals = this.options.locals = _.merge( this.options.locals, ( options.locals || {} ) );
  ...
  Sitter.prototype.compileContent = function compileContent( content, options, cb ){
    ...
    options.locals = _.merge( _.merge( {}, this.options.locals ), (options.locals || {}) );
    options.prettyHtml = this.options.prettyHtml;

    if( this.options.controller && typeof( this.options.controller.before ) === 'function' )
      return this.options.controller.before( content, options, function(){
    ...
    var templateFiles = fs
      ...
      .filter( function( file ){ 
        ...
        if( basename(file, extname(file)) === options.template+'.controller' && extname(file) === '.js' ){
          ...
          self.options.controller = require( join(templatePath,file) );
        }
  ...
  Sitter.prototype.compileObject = function compileObject( obj, options, cb ){
    ...
    if( !this.options.locals.doc )
      this.options.locals.doc = ( options.locals.doc && options.locals.doc ) || obj;
   ...
   * @private
   */
  Sitter.prototype.compileObjectTranslations = function compileObjectTranslations( obj, options, cb ){
    var compiler = this;
    ...
    function processEachTranslation( obj, options ){
      ...
      // if this.options.multiLangs is true (default)
      if( compiler.options.defaultLocale && !compiler.options.multiLangs )
        return cb();
      
      async.eachSeries( obj.translations, function( translation, nextTranslation ){
        options.locals.doc = obj;
        options.locals.doc.curLang = options.curLang = translation.locale;
   ...
   */
  Sitter.prototype.compileDefaultLocaleCopy = function compileDefaultLocaleCopy( obj, options, cb ){
    var translation = _.find( obj.translations, { locale: this.options.defaultLocale });
    if( !translation )
  ...
  Sitter.prototype.writeToDestination = function writeToDestination( htmlContent, options, cb ){
    ...
    if( options.filenamePrefix && options.filename )
      options.filename = join( options.filenamePrefix, options.filename );
    Sitter.writers[protocol]( htmlContent, (options.filename || null), _.merge( {  cwd: this.cwd, 
